...
class LLM2Vec(nn.Module):
    # 模型定义（模型、分词器、池化模式、模型输入硬性上限、在分词前用于缩减文本的软性上限、编码时是否不包含指令）
    def __init__(
        self,
        model: AutoModel,
        tokenizer: AutoTokenizer,
        # 如何从模型的最后一层隐藏状态中汇总信息
        ## mean: 取平均池化，将所有令牌的隐藏状态取平均值。这种方式适合平衡地整合整个句子的信息
        ## weighted_mean: 加权平均池化，对每个令牌的隐藏状态赋予权重，位置越靠后的令牌权重越高（权重递增），这样可以让模型更关注句尾内容
        ## eos_token / last_token: 取最后一个令牌的隐藏状态作为句子嵌入，适合一些基于序列结束符的生成任务
        ## bos_token: 取开头的特殊令牌（如<bos>）的隐藏状态。适合一些需要关注句子起始信息的任务
    ...
    # 辅助函数，根据模型的配置类名称 config_class_name 和启用双向模型 enable_bidirectional 选择适当的模型类
    @classmethod
    ...
    # forward过程中执行的一个步骤，将attention_mask调整为embed_mask，从而实现跳过指令内容的功能
    ...
    # 神经网络的前向传播步骤，接受输入特征sentence_feature，通过模型计算并生成隐藏层状态。然后调用get_pooling方法来应用所选的池化方式，最终返回经过池化的嵌入表示
    ...
    # 执行嵌入的池化操作。它根据pooling_mode的不同选择不同的池化策略
    ...
    # 帮助函数，负责将指令和文本内容合并为一个字符串，且确保文本长度不超过doc_max_length
    ...
    # 保存模型、分词器和配置文件。提供了一个可选参数merge_before_save，用于在保存前合并PEFT模型
    ...
    # 辅助方法，用于对批量句子进行编码。它会调用tokenize函数对输入文本进行分词和设备迁移，然后在forward函数中生成嵌入表示，返回处理后的嵌入。此方法可以单独用于多进程编码
    def _encode(
        self,
        sentences_batch,
        device: Optional[str] = None,
    ...
    # 帮助函数，用于获取输入文本的长度，支持多种输入形式，便于在编码前检查文本长度
    ...
    # 调整模型的嵌入大小，以适应新词汇表的需求或按特定倍数进行填充（用于对齐）
    ...
    # 启用梯度检查点功能，用于减少显存占用（特别是在处理大规模模型时），但会略微降低训练速度
